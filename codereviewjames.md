...
tsawait itemRepository.update(id, { status: 'completed' });
...
tsawait knex.transaction(async (trx) => {
...
tsexport async function deleteItem(id: number): Promise<void> {
  await knex('items').where({ id }).delete(); // no ownership check
}

After:

tsexport async function deleteItem(id: number, userId: number): Promise<boolean> {
